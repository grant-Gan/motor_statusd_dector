...
二、预期效果
  ...
  Vibration value is :  CH0 xx  CH1 xx  CH2 xx  CH3 xx
  其中xx的取值范围为 0 -- 20, 保留两位小数  
*/
...
void loop() {
    //更新振动传感器状态
  motor.updateVibra();  
  Serial.print("Vibration value is :");
  Serial.print("\tCH0 : ");Serial.print(motor.getVibraChx(motor.CH0));
  Serial.print("\tCH1 : ");Serial.print(motor.getVibraChx(motor.CH1));
  Serial.print("\tCH2 : ");Serial.print(motor.getVibraChx(motor.CH2));
  Serial.print("\tCH3 : ");Serial.print(motor.getVibraChx(motor.CH3));
  Serial.println();
  delay(2000);
} 
